...
)
{
	...
	{
		double defense = armor->defense();
		if (defense > 0 && defense > min_defense && defense < max_defense)
			temp.push_back(armor);

		if((temp.size())==total_size)
			return std::make_unique<ArmorVector>(temp);
	}
return std::make_unique<ArmorVector>(temp);
...
)
{
	...
	while (!todo.empty())
	{
		...
		{
			if ((( todo[x]->defense()/todo[x]->cost()) > best) && (result_cost+todo[x]->cost())<=total_cost)
			{
					best = (todo[x]->defense() / todo[x]->cost());
					g = todo[x]->cost();
					a=x;
			}
		}

		if ((result_cost + todo[a]->cost()) <= total_cost)
		{
			result.push_back(todo[a]);
			result_cost += g;
		}
		todo.erase(todo.begin()+a);
	}
	return std::make_unique<ArmorVector>(result);
}
...
)
{
	...
	ArmorVector best;
	double totalGold= 0.0, totalDef=0.0,totalDefB =0.0;

	for (auto bits = 0; bits < pow(2, n); bits++)
	{
		...
		for (auto j = 0; j < n; j++)
		{
			if ( ((bits >> j) & 1) == 1)
			{
				...
				sum_armor_vector(candidate,totalGold,totalDef);
			}
		}
		if (totalGold <= total_cost)
		{
			if ((best.empty() || totalDef > totalDefB) && candidate.size()>0)
			{
				best = candidate;
				totalDefB = totalDef;
			}
	...
	return std::make_unique<ArmorVector>(best);
}
